...
void print_usage(const char *program_name) {
    ...
    printf("      --io-limit <dev:rbps:wbps> I/O limit in bytes/sec (e.g., 8:0:10485760)\n");
    printf("\n");
...
int run_command_in_cgroup(int argc, char *argv[], const char* cgroup_name, double cpu_limit, uint64_t mem_limit_mb, const char *io_limit_str) {
    if (geteuid() != 0) {
    ...
    char io_cgroup_path[PATH_MAX] = "";
    if (io_limit_str != NULL) {
        char device[64];
        uint64_t rbps = 0, wbps = 0;
        // Parse dev:rbps:wbps
        if (sscanf(io_limit_str, "%63[^:]:%lu:%lu", device, &rbps, &wbps) == 3) {
            // For cgroup v1, we need to find the blkio path
            if (version == 1) {
                snprintf(io_cgroup_path, sizeof(io_cgroup_path), "/sys/fs/cgroup/blkio/%s", final_cgroup_name);
                mkdir(io_cgroup_path, 0755); // Ensure it exists
            } else {
                strncpy(io_cgroup_path, cpu_cgroup_path, sizeof(io_cgroup_path));
            }
            if (set_cgroup_io_limit(io_cgroup_path, device, rbps, wbps) == 0) {
                printf("✓ I/O limit set on device %s (R: %lu B/s, W: %lu B/s).\n", device, rbps, wbps);
            } else {
                fprintf(stderr, "Error setting I/O limit: %s\n", strerror(errno));
            }
        } else {
            fprintf(stderr, "Warning: Could not parse I/O limit string: %s\n", io_limit_str);
        }
    }
    ...
    if (child_pid == 0) { // Child process
        ...
        if (move_process_to_cgroup(getpid(), cpu_cgroup_path) != 0) {
            perror("Failed to move child to cpu cgroup");
            exit(EXIT_FAILURE);
        }
        if (move_process_to_cgroup(getpid(), mem_cgroup_path) != 0) {
            perror("Failed to move child to memory cgroup");
            exit(EXIT_FAILURE);
        }
        if (strlen(io_cgroup_path) > 0 && move_process_to_cgroup(getpid(), io_cgroup_path) != 0) {
            perror("Failed to move child to io cgroup");
            exit(EXIT_FAILURE);
        }
        execvp(argv[0], argv);
...
int main(int argc, char *argv[]) {
    ...
    static struct option long_options[] = {
        ...
        {"io-limit",    required_argument, 0, 259},
        {0, 0, 0, 0}
    };
    ...
    if (double_dash_index != -1) {
        ...
        return run_command_in_cgroup(argc - double_dash_index - 1, &argv[double_dash_index + 1], cgroup_name, cpu_limit, mem_limit_mb, io_limit_str);
    } else {
        // Monitoring Mode
        if (optind >= argc) {
        ...
        double sample_latency_ms = (end_time.tv_sec - start_time.tv_sec) * 1000.0 +
                                   (end_time.tv_nsec - start_time.tv_nsec) / 1000000.0;

        if (quiet) {
            printf("sample_latency_ms:%.4f\n", sample_latency_ms);
        }
